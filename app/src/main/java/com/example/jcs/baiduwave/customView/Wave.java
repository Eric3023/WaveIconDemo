package com.example.jcs.baiduwave.customView;

import android.animation.ValueAnimator;
import android.content.Context;
import android.content.res.TypedArray;
import android.graphics.Bitmap;
import android.graphics.Canvas;
import android.graphics.Color;
import android.graphics.Paint;
import android.graphics.Path;
import android.graphics.PorterDuff;
import android.graphics.PorterDuffXfermode;
import android.graphics.Rect;
import android.graphics.RectF;
import android.graphics.Typeface;
import android.util.AttributeSet;
import android.util.Log;
import android.view.View;
import android.view.animation.LinearInterpolator;

import com.example.jcs.baiduwave.R;
import com.example.jcs.baiduwave.tools.DimentionUtils;


/**
 * author：Jics
 * 2017/7/6 13:06
 */
public class Wave extends View {
    private Paint mPaint;
    private Paint textPaint;
    private int mWidth = DimentionUtils.dip2px(getContext(), 50);
    private int mHeight = DimentionUtils.dip2px(getContext(), 50);
    private Path path;
    private float currentPercent;
    private int backgroundColor;
    private int waveColor;
    private int backgroudTextColor;
    private int waveTextColor;
    private String text = "贴";
    private int textSize = DimentionUtils.sp2px(getContext(), 25);

    public Wave(Context context) {
        this(context, null);
    }

    public Wave(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public Wave(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context,attrs);

    }

    private void init(Context context, AttributeSet attrs) {
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.Wave);
        //自定义颜色和文字
        backgroundColor = array.getColor(R.styleable.Wave_backgroundcolor, Color.TRANSPARENT);
        waveColor = array.getColor(R.styleable.Wave_wavecolor, Color.rgb(41, 163, 254));
        text = array.getString(R.styleable.Wave_text);
        textSize = (int) array.getDimension(R.styleable.Wave_textsize, 48);
        backgroudTextColor=array.getColor(R.styleable.Wave_backgroundtextcolor, Color.rgb(41, 163, 254));
        waveTextColor=array.getColor(R.styleable.Wave_wavetextcolor, Color.WHITE);
        array.recycle();
        //图形及路径填充画笔
        mPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mPaint.setStyle(Paint.Style.FILL);
        mPaint.setDither(true);
        //文字画笔
        textPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        textPaint.setTypeface(Typeface.DEFAULT_BOLD);
        textPaint.setTextSize(textSize);
        //闭合波浪路径
        path = new Path();

        ValueAnimator animator = ValueAnimator.ofFloat(0, 1);
        animator.setDuration(1000);
        animator.setInterpolator(new LinearInterpolator());
        animator.setRepeatCount(ValueAnimator.INFINITE);
        animator.setRepeatMode(ValueAnimator.RESTART);
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                currentPercent = animation.getAnimatedFraction();
                invalidate();
            }
        });
        animator.start();
    }

    @Override
    protected void onDraw(Canvas canvas) {
        //裁剪成圆形
        Path o = new Path();
        o.addCircle(mWidth / 2, mHeight / 2, mWidth / 2, Path.Direction.CCW);
//        canvas.clipPath(o);
        //底层颜色
        mPaint.setColor(backgroundColor);
        canvas.drawPath(o, mPaint);
        //底部的字
        textPaint.setColor(backgroudTextColor);
        drawCenterText(canvas, textPaint, text);
        //生成闭合波浪路径
        path = getActionPath(currentPercent);
        //画波浪
        mPaint.setColor(waveColor);
//        canvas.drawPath(path, mPaint);
        //裁剪路径
        canvas.clipPath(path);
        canvas.drawPath(o, mPaint);
        //上层的字
        textPaint.setColor(waveTextColor);
        drawCenterText(canvas, textPaint, text);
    }

    @Override
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        int widthSize = MeasureSpec.getSize(widthMeasureSpec);
        int widthMode = MeasureSpec.getMode(widthMeasureSpec);
        int heightSize = MeasureSpec.getSize(heightMeasureSpec);
        int heightMode = MeasureSpec.getMode(heightMeasureSpec);
        if (widthMode == MeasureSpec.EXACTLY) {
            mWidth = widthSize;
        }
        if (heightMode == MeasureSpec.EXACTLY) {
            mHeight = heightSize;
        }
        setMeasuredDimension(mWidth, mHeight);
    }

    private Path getActionPath(float percent) {
        Path path = new Path();
        int x = -mWidth;
        //当前x点坐标（根据动画进度水平推移，一个动画周期推移的距离为一个mWidth）
        x += percent * mWidth;
        //波形的起点
        path.moveTo(x, mHeight / 2);
        //控制点的相对宽度
        int quadWidth = mWidth / 4;
        //控制点的相对高度
        int quadHeight = mHeight / 5;
        //第一个周期
        path.rQuadTo(quadWidth/2, quadHeight, quadWidth * 2, 0);
        path.rQuadTo(quadWidth*3/2, -quadHeight, quadWidth * 2, 0);
        //第二个周期
        path.rQuadTo(quadWidth/2, quadHeight, quadWidth * 2, 0);
        path.rQuadTo(quadWidth*3/2, -quadHeight, quadWidth * 2, 0);
        //右侧的直线
        path.lineTo(x + mWidth * 2, mHeight);
        //下边的直线
        path.lineTo(x, mHeight);
//        path.addCircle(mWidth/2, mHeight/2, mHeight, Path.Direction.CW);
        //自动闭合补出左边的直线
        path.close();
        return path;
    }

    private void drawCenterText(Canvas canvas, Paint textPaint, String text) {
        Rect rect = new Rect(0, 0, mWidth, mHeight);
        textPaint.setTextAlign(Paint.Align.CENTER);

        Paint.FontMetrics fontMetrics = textPaint.getFontMetrics();
        float top = fontMetrics.top;
        float bottom = fontMetrics.bottom;

        int centerY = (int) (rect.centerY() - bottom/2 -top/ 2);
        canvas.drawText(text, rect.centerX(), centerY, textPaint);
    }
}
